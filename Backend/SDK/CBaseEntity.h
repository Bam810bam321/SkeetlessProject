...
{\
	return *( t * )( (uintptr_t)this + o );\
		...
		OFFSET(int, get_bone_count, 0x2690)
		OFFSET( matrix3x4_t, bone_cache, 0x26A8)
		OFFSET(int, eflags, 0x104)
		OFFSET(Vector, abs_velocity, 0x114)
		OFFSET(int, last_setupbones_frame, 0x2928)
		...
		{
			if ( this )
				return ( Collideable* )( ( DWORD )this + 0x320);
		...
		CUserCmd*& m_pCurrentCommand() {
			static auto current_command = *(uint32_t*)(Cheat::Utilities->Memory_PatternScan(xorstr_("client.dll"), xorstr_("8D 8E ? ? ? ? 89 5C 24 3C")) + 2);
		...
		void SetAngle2(Vector wantedang)
		{
			...
			static SetAngleFn SetAngle = (SetAngleFn)((DWORD)Cheat::Utilities->Memory_PatternScan(xorstr_("client.dll"), xorstr_("55 8B EC 83 E4 F8 83 EC 64 53 56 57 8B F1")));
...
#define	  PlayerVisibleMask				(CONTENTS_HITBOX | CONTENTS_MONSTER | CONTENTS_DEBRIS | CONTENTS_MOVEABLE | CONTENTS_SOLID | CONTENTS_WINDOW | CONTENTS_GRATE)
		...
		static void UpdateAnimationState(CBaseAnimState *state, Vector angle)
		{
			...
			static auto UpdateAnimState = Cheat::Utilities->Memory_PatternScan(xorstr_("client.dll"), xorstr_("55 8B EC 83 E4 F8 83 EC 18 56 57 8B F9 F3"));
			if (!UpdateAnimState)
				return;
			...
			static auto oUpdateAnimationState = (UpdateAnimationStateFn)(Utilities->Memory_PatternScan("client.dll", "55 8B EC 83 E4 F8 83 EC 18 56 57 8B F9 F3")); // @xref: "%s_aim"
		...
		void CreateAnimationState(CBaseAnimState *state)
		{
			...
			static auto CreateAnimState = (CreateAnimState_t)Cheat::Utilities->Memory_PatternScan(xorstr_("client.dll"), xorstr_("55 8B EC 56 8B F1 B9 ? ? ? ? C7"));
			if (!CreateAnimState)
				return;
		...
		{
			if ( this )
				return reinterpret_cast<Collideable*>( ( DWORD )this + 0x320);
		}

		VMatrix &GetCollisionBoundTrans()
		{
			if ( this )
				return *reinterpret_cast<VMatrix*>( ( DWORD )this + 0x474);
		}
		...
		{
			*reinterpret_cast<CBaseAnimState**>( uintptr_t( this ) + 0x3268) = state;
		}
		...
		void InvalidateBoneCache()
		{
			...
			static auto func = (OriginalFn)Utilities->Memory_PatternScan("client.dll", "80 ? ? ? ? ? ? 74 16 A1 ? ? ? ? 48 C7 ? ? ? ? ? ? ? ? ? 89 ? ? ? ? ? C3");
