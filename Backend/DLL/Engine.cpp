...
void SetupOffsets()
{
	...
	offys.dwSetAbsOrigin = Cheat::Utilities->Memory_PatternScan( Cheat::Settings->ClientDLL, xorstr_("55 8B EC 83 E4 F8 51 53 56 57 8B F1 E8 ? ? ? ? 8B 7D") );
	offys.dwSetAbsAngles = Cheat::Utilities->Memory_PatternScan( Cheat::Settings->ClientDLL, xorstr_("55 8B EC 83 E4 F8 83 EC 64 53 56 57 8B F1") );
	offys.dwLoadFromBuffer = Utilities->Memory_PatternScan( Cheat::Settings->ClientDLL, ( xorstr_("55 8B EC 83 E4 F8 83 EC 34 53 8B 5D 0C 89") ) );
	offys.dwInitKeyValues = Utilities->Memory_PatternScan( Cheat::Settings->ClientDLL, ( xorstr_("55 8B EC 51 33 C0 C7 45") ) ) - 0x45;
	offys.dwSmokeCount = Cheat::Utilities->Memory_PatternScan( Cheat::Settings->ClientDLL, xorstr_("A3 ? ? ? ? 57 8B CB") ) + 1;
	offys.dwLoadSky = Utilities->Memory_PatternScan( xorstr_("engine.dll"), xorstr_("55 8B EC 81 ? ? ? ? ? 56 57 8B F9 C7") );
	offys.dwCrosshair = Cheat::Utilities->Memory_PatternScan( Cheat::Settings->ClientDLL, xorstr_("83 F8 05 75 17") );
	offys.dwSetClanTag = Cheat::Utilities->Memory_PatternScan( xorstr_("engine.dll"), xorstr_("53 56 57 8B DA 8B F9 FF") );
	offys.dwServerRankRevealAll = Cheat::Utilities->Memory_PatternScan( Cheat::Settings->ClientDLL, xorstr_("55 8B EC 51 A1 ? ? ? ? 85 C0 75 37") );
...
bool Cheat::Initialize()
{
	...
	if ( !Settings->UIDevMode )
	{
		...
		static auto dwDoExtraBonesProcessing = (DWORD)((Utilities->Memory_PatternScan("client.dll", "55 8B EC 83 E4 F8 81 ? ? ? ? ? 53 56 8B F1 57 89 74 24 1C")));
		static auto dwShouldSkipAnimFrame = (DWORD)(Utilities->Memory_PatternScan("client.dll", "57 8B F9 8B 07 8B ? ? ? ? ? FF D0 84 C0 75 02")); // 57 8B F9 8B 07 8B 80 ? ? ? ? FF D0 84 C0 75 02
		...
		static auto dwMove = (DWORD)((Utilities->Memory_PatternScan("engine.dll", "55 8B EC 81 EC ? ? ? ? 53 56 8A F9")));
